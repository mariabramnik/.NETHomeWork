...
namespace HomeWork32MsSQL
{
    class Program
    {
        //check that the number that user entered is integer and this number  is on the list
        public static int CheckForInt(string str)
        {
            int num = 0;
            bool res = false;
            res = int.TryParse(str, out num);
            while (res == false || (num < 1 || num > 11 || num == 4))
            {
        ...
        static void Main(string[] args)
        {
            ...
            while (num != 11)
            {
                ...
                Console.WriteLine("11 - Press To Exit ");
                
                string answer = Console.ReadLine();
                //check that entered number is in the list
                num = CheckForInt(answer);
                string myStr = "";
                // current string myStr will be equal to one of queries
                switch (num)
                {
                    case 1:
                        myStr = str1;
                ...
                //read answers depending on queries
                switch (num)
                {
                    case 1:
                        while (reader.Read())
